...
// elas 
        int elasNumDisparities; 
	fs["ELAS.SGBM.numDisparities"]  >> elasNumDisparities;
        m_maxDisparies_elas = elasNumDisparities;
	elas.param.disp_min = 0;
	elas.param.disp_max = elasNumDisparities;//最大视差
 ...
 * 参数 : disparity		[out]	视差图
 */
int StereoMatch::elasMatch(cv::Mat& frameLeft, cv::Mat& frameRight, cv::Mat& disparity)
{
	// 输入检查
	if (frameLeft.empty() || frameRight.empty())
	{
	...
	// BM只能处理单通道图像　转换为灰度图
	cv::Mat imgLproc, imgRproc;
	if(frameLeft.channels()==3) cv::cvtColor(frameLeft,  imgLproc, COLOR_BGR2GRAY);
        else imgLproc = frameLeft;
	if(frameRight.channels()==3) cv::cvtColor(frameRight, imgRproc, COLOR_BGR2GRAY);
        else imgRproc = frameRight;
	// 校正图像，使左右视图行对齐
...
*/	imgLremap = imgLproc;
	...
	//	m_numberOfDisparies = m_ptr_BM->params.numDisparities;
	cv::copyMakeBorder( imgLremap, imgLborder, 0, 0, 
			m_maxDisparies_elas, 0, IPL_BORDER_REPLICATE);// cv::BORDER_REPLICATE
	cv::copyMakeBorder( imgRremap, imgRborder, 0, 0, 
			m_maxDisparies_elas, 0, IPL_BORDER_REPLICATE);
        m_numberOfDisparies = m_maxDisparies_elas;
	// 计算视差 带有扩展的边框
	const cv::Size imsize = imgLborder.size();
	...
	elas.process(imgRborder.data,imgRborder.data,leftdpf.ptr<float>(0),rightdpf.ptr<float>(0),dims);

	// 截取与原始画面对应的视差区域（舍去加宽的部分）
	cv::Mat disp32;
	disp32 = leftdpf.colRange(m_numberOfDisparies, imgLborder.cols);	
        disp32.convertTo(disparity,CV_16S,16);
        //disp32.copyTo(disparity);
