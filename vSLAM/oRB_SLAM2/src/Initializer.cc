...
T =  K 逆 * H21*K
  ...
  *  * p1------> p2
 *                           f1   f2    f3      u2
 *   (u1 v1 1)    *   f4   f5    f6  *  v2    = 0  应该=0 不等于零的就是误差
 * 			      f7   f8    f9	     1
 * 	a1 = f1*u1 + f2*v1 + f3;
	b1 = f4*u1 + f5*v1 + f6;
	c1 = f7*u1 + f8*v1 + f9;
...
// 单目相机初始化   参考帧
	Initializer::Initializer(const Frame &ReferenceFrame, float sigma, int iterations)
	{
       ...
       // 当前帧  匹配点对  旋转矩阵  平移矩阵  3d点
	bool Initializer::Initialize(const Frame &CurrentFrame, const vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
				    vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
	{
	    // Fill structures with current keypoints and matches with reference frame
	    // Reference Frame: 1, Current Frame: 2
	    mvKeys2 = CurrentFrame.mvKeysUn;// 畸变校正后的 关键 点

	    mvMatches12.clear();// 匹配点对
	    mvMatches12.reserve(mvKeys2.size());
	    mvbMatched1.resize(mvKeys1.size());// 匹配信息
	    for(size_t i=0, iend=vMatches12.size();i<iend; i++)
	    {
		if(vMatches12[i]>=0)// 有匹配
		{
		...
		for(size_t j=0; j<8; j++)
		{
		    ...
		    int idx = vAvailableIndices[randi];// 对于的随机数
 ...
 // 【2】随机采样序列迭代求解
   ...
   //【3】随机8对点对
    ...
    // 【4】计算 单应矩阵        T1*p1  ----> T2*p2     p1 ----------------> p2
		...
		*                                                            				   最高分 均为 5.99 
		*/
     // 【5】计算单应H的得分  有由 对应的匹配点对	的 对称的转换误差 求得	
...
// 【3】恢复四种假设 并验证 Reconstruct with the 4 hyphoteses and check
