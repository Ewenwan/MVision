...
2  3个 3*3卷积核  64输出的参差模块 256/4 = 64     且第一个参差块的第一个卷积步长为1
3  4个 3*3卷积核  128输出的参差模块 512/4 = 128   且第一个参差块的第一个卷积步长为2      
4  6个 3*3卷积核  256输出的参差模块 1024/4 = 256  且第一个参差块的第一个卷积步长为2  
5  3个  3*3卷积核 512输出的参差模块 2048/4 = 512  且第一个参差块的第一个卷积步长为2  
6  均值池化 
7  全连接层 输出 1000  类
8  softmax 类别预测
...
class ResNet50(object):
    ...
    def _block(self, x, n_out, n, init_stride=2, is_training=True, scope="block"):
        with tf.variable_scope(scope):
            ...
            # 第一个参差模型（会涉及到 不同参差集合块 不同通道的合并） f(x) + x
            # 这里第一个参差块的 卷积步长 与后面的参差块的卷积步长可能不一样 所以独立出来
            out = self._bottleneck(x, h_out, n_out, stride=init_stride,
                                   is_training=is_training, scope="bottlencek1")
            for i in range(1, n):#1....(n-1)个参数块
