...
void BaseRistrettoLayer<Dtype>::QuantizeWeights_cpu(
      vector<shared_ptr<Blob<Dtype> > > weights_quantized, // 权重w 和 偏执b
      const int rounding,  // 量化取整策略
      const bool bias_term) 
{
   //float scale =0.3;
   //uint8_t zero_point = 60;
  Dtype* weight = weights_quantized[0]->mutable_cpu_data();// 权重 w
  ...
  case QuantizationParameter_Precision_MINIFLOAT:
   ...
   // if (bias_term) 
	//{// Don't trim bias  bias的范围和weight的范围有时候不一致，造成较大的损失
  ...
  case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
   ...
   // if (bias_term) {// Don't trim bias  bias的范围和weight的范围有时候不一致，造成较大的损失
...
// IAO量化
  case QuantizationParameter_Precision_INTEGER_ARITHMETRIC_ONLY:
    Trim2IAO_cpu(weight, cnt_weight, scale_params_, zero_point_params_);
    // Don't trim bias
    break;
...
// IAO量化
    case QuantizationParameter_Precision_INTEGER_ARITHMETRIC_ONLY:
      Trim2IAO_cpu(data, count, scale_in_, zero_point_in_);
    // Don't trim bias
      break;
...
// IAO量化===============================
    case QuantizationParameter_Precision_INTEGER_ARITHMETRIC_ONLY:
      Trim2IAO_cpu(data, count, scale_out_, zero_point_out_);
    // Don't trim bias
      break;
...
// IAO整数 uint8 量化===========================================
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::Trim2IAO_cpu(Dtype* data, const int cnt, float scale, uint8_t zero_point){
   for (int index = 0; index < cnt; ++index) {
    const Dtype real_val = data[index];
    const Dtype transformed_val = zero_point + real_val / (Dtype)scale;
    const Dtype clamped_val = (Dtype)std::max((Dtype)0, std::min((Dtype)255, transformed_val));// 限制幅度 0.0 ~ 255.0 之间
    //data[index] = static_cast<uint8_t>round(clamped_val);
	
    // data[index] = round(clamped_val);
	data[index] = round(clamped_val) - zero_point; // 量化后再减去零点
   }
}

// IAO整数 卷积整数结果反变成 浮点结果============================
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::IAO_Conv_Int_Res_2_real_cpu(Dtype* data, const int cnt){
  
  const Dtype real_multiplier = scale_in_ * scale_params_ / scale_out_;
  int right_shift;// 除去 左移31位，原数的扩大倍数记录
  int32_t quantized_multiplier;// 量化后的整数
  QuantizeMultiplierSmallerThanOne(real_multiplier, &quantized_multiplier, &right_shift);
  
   for (int index = 0; index < cnt; ++index) 
   {
    const Dtype qual_val = data[index];
    const Dtype transformed_val = zero_point_out_ + ((qual_val * (Dtype)quantized_multiplier)>>right_shift);
    const Dtype clamped_val = (Dtype)std::max((Dtype)0, std::min((Dtype)255, transformed_val));// 限制幅度 0.0 ~ 255.0 之间
    //data[index] = static_cast<uint8_t>round(clamped_val);
    
    // data[index] = round(clamped_val);
    data[index] = round(clamped_val); // 量化后再减去零点
   }
   IaoDequantize_cpu(data, cnt);
}


// IAO整数 uint8 反量化===========================================
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::IaoDequantize_cpu(Dtype* data, const int cnt, float scale, uint8_t zero_point) 
{
   for (int index = 0; index < cnt; ++index) {
    const Dtype quantized_val = data[index];
    data[index] = (Dtype)(scale * (quantized_val - zero_point));
   }
}

// 小于1的小数 转换成 32位的整数
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::QuantizeMultiplierSmallerThanOne(Dtype real_multiplier, // 实际小数 乘数
                                      int32_t* quantized_multiplier,
                                      int* right_shift) 
{
// 确保在 0.0~1.0 之间==================                              
  CHECK_GT(real_multiplier, (Dtype)0);
  CHECK_LT(real_multiplier, (Dtype)1);
// 改变范围 到 0.5~1 之间===============
  int s = 0;// 扩大倍数 记录， 之后右移相同的数量，就会还原
  while (real_multiplier < (Dtype)0.5) 
  {
    real_multiplier *= (Dtype)2.0;
...
// 如果原数 real_multiplier 比较趋近于1，将其减半，同时扩大倍数记录-1
  ...
  CHECK_GE(s , 0);
  //CHECK_LE(q <= std::numeric_limits<std::int32_t>::max());
