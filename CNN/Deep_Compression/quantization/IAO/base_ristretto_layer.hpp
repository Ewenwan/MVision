...
// IAO量化=============================
// IAO整数 uint8 量化===========================================
  void Trim2IAO_cpu(Dtype* data, const int cnt, float scale, uint8_t zero_point); 
  void Trim2IAO_gpu(Dtype* data, const int cnt, float scale, uint8_t zero_point);

  // 反量化
 void IaoDequantize_cpu(Dtype* data, const int cnt, float scale, uint8_t zero_point);
 void IaoDequantize_gpu(Dtype* data, const int cnt, float scale, uint8_t zero_point);
 // 小于1的小数 转换成 32位的整数
 void QuantizeMultiplierSmallerThanOne(Dtype real_multiplier,
                                      int32_t* quantized_multiplier,
                                      int* right_shift);

// IAO整数 卷积整数结果反变成 浮点结果======
void IAO_Conv_Int_Res_2_real_cpu(Dtype* data, const int cnt);
  ...
  // IAO 量化参数 
  float scale_in_, scale_params_, scale_out_;
  uint32_t zero_point_in_, zero_point_params_, zero_point_out_;
};
