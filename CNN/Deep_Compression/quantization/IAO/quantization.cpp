...
void Quantization::QuantizeNet() 
{
  ...
  RunForwardBatches(10, net_test, &accuracy, true);// 记录网络每一层的量化参数
  delete net_test;
  ...
  else if(trimming_mode_ == "integer-arithmetic-only")
  {
    Quantize2IntegerArithmeticOnly();
  }
  else
  {
      ...
      const int score_number) 
{
  ...
  for (int i = 0; i < iterations; ++i) 
  {
    ...
    if(do_stats) 
    {
      caffe_net->RangeInLayers(&layer_names_, &abs_max_in_, &abs_max_out_,&abs_max_params_, 
                               &max_in_, &min_in_, &max_out_, &min_out_, &max_params_, &min_params_);
    }
...
void Quantization::Quantize2IntegerArithmeticOnly()
{
 ...
 //=====计算每一层的量化参数==========================
  ...
  // 偏移量 零点===================
   ...
   ChooseIAOQuantizationParams(min_in_[i],  max_in_[i],  &zero_point_in_[i],  &scale_in_[i]);
   ChooseIAOQuantizationParams(min_out_[i], max_out_[i], &zero_point_out_[i], &scale_out_[i]);
   ChooseIAOQuantizationParams(min_params_[i], max_params_[i], &zero_point_params_[i], &scale_params_[i]);
  }
  ...
  {
    LOG(INFO) << "Layer " << layer_names_[k] <<
        ", zero point input=" << zero_point_in_[k] << " scale input = " << scale_in_[k] << 
        ", zero point output=" << zero_point_out_[k] << " scale output = " << scale_out_[k] << 
        ", zero point parameters=" << zero_point_params_[k]<< " scale parameters = " << scale_params_[k];
  }
...
// 修改网络 并测试
 ...
 // net_test = new Net<float>(param);
  ...
  // RunForwardBatches(iterations_, net_test, &accuracy); // 需要 特点的iao层来支持网络前传
  //delete net_test;
