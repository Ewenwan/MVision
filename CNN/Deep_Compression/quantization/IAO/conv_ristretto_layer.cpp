...
// 层类 构造函数函数================= 声明在 base_ristretto_layer.hpp
      ...
      BaseRistrettoLayer<Dtype>() // 继承于 BaseRistrettoLayer 和 ConvolutionLayer
{
  ...
  {
  case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:// 动态固定点量化 
  ...
  case QuantizationParameter_Precision_INTEGER_ARITHMETRIC_ONLY:// IAO量化==============================
    this->scale_in_ = this->layer_param_.quantization_param().scale_in();
    this->scale_params_ = this->layer_param_.quantization_param().scale_params();
    this->scale_out_ = this->layer_param_.quantization_param().scale_out();
    this->zero_point_in_ = this->layer_param_.quantization_param().zero_point_in();
    this->zero_point_params_ = this->layer_param_.quantization_param().zero_point_params();
    this->zero_point_out_ = this->layer_param_.quantization_param().zero_point_out();
    break;
...
{
if(this->precision_ == QuantizationParameter_Precision_INTEGER_ARITHMETRIC_ONLY)
{
 if (this->phase_ == TEST) // 测试阶段 
  {
	  for (int i = 0; i < bottom.size(); ++i) 
		{
		  // base_ristretto_layer.cpp 中  // 测试阶段 会量化 输入 
		  this->QuantizeLayerInputs_cpu(bottom[i]->mutable_cpu_data(),bottom[i]->count());// 量化激活输入====
		}
	
	  // 拷贝一份 权重w 和 偏置b 用于量化======================
	  caffe_copy(this->blobs_[0]->count(), this->blobs_[0]->cpu_data(),this->weights_quantized_[0]->mutable_cpu_data());
  ...
  for (int i = 0; i < bottom.size(); ++i) // 多个数据来源=======
  {
    const Dtype* bottom_data = bottom[i]->cpu_data();// 层输入数据
	
    Dtype* top_data = top[i]->mutable_cpu_data();    // 输出数据
    
	//  特征图 N*C1*W*H  * 卷积核 C1*w*h*C2 C2为输出通道数 ---> N * C2 * W * H
    for (int n = 0; n < this->num_; ++n) // 多个batch 
    {// bottom_dim_ = 输入度维度 = C1*W*H
	 // top_dim_ = 输出维度 = 输出通道数*输出h*输出w = 

      this->forward_cpu_gemm(bottom_data + n * this->bottom_dim_, weight,
          top_data + n * this->top_dim_);// 卷积运算

      // 将整数运算结果反量化回 浮点数
      this->IAO_Conv_Int_Res_2_real_cpu(top_data + n * this->top_dim_, this->top_dim_ );
   ...
   // 下一层也会进行量化，再则之前统计参数大小时，是按照浮点数进行统计的
...
// 训练阶段全精度=============================================
 ...
 {
  // Do forward propagation
  const Dtype* weight = this->blobs_[0]->cpu_data();// 未量化 的 卷积权重w
  
  for (int i = 0; i < bottom.size(); ++i) // 底部 输入的 特征图 激活值
  {
    const Dtype* bottom_data = bottom[i]->cpu_data();// 层输入数据
      ...
      const vector<Blob<Dtype>*>& bottom) 
 {
  ...
  for (int i = 0; i < top.size(); ++i) 
  {
    ...
    if (this->bias_term_ && this->param_propagate_down_[1]) 
	{
      ...
      for (int n = 0; n < this->num_; ++n) // batchsize
	  {
