...
#include <boost/property_tree/json_parser.hpp>// 解析json
#include <boost/property_tree/ptree.hpp>// 数据结构
#include <boost/property_tree/xml_parser.hpp>// 解析
#include <fcntl.h>
...
const int height,  // 指定缩放后的大小
const int width, 
const int min_dim, // 两者其一  指定 最小值 确定图像缩放因子
const int max_dim,
const bool is_color) 
{
// 安装 彩色/灰度 读取图像
...
{
  // Read image to datum.
  // 生成图像 像素值的 anno_datum->mutable_datum() -> datum 
...
// 转换标签数据 到 anno_datum
  switch (type) {
    case AnnotatedDatum_AnnotationType_BBOX:
	  // 获取原图像尺寸====================
      int ori_height, ori_width;// 原图像大小
...
const int img_width, 
const std::map<string, int>& name_to_label, // 标注文件 name标签 : class_id
AnnotatedDatum* anno_datum) // 数据格式
{
  ptree pt;// 优先树
...
//
  ...
  BOOST_FOREACH(ptree::value_type &v1, pt.get_child("annotation"))
 {
    ...
    if (v1.first == "object") 
	{
	  ...
	  {
        ptree pt2 = v2.second;
/////name 域 物体类别   需要分组  yolo9000  
        if (v2.first == "name") 
		{
          string name = pt2.data();// 标注文件中的 边框 类别 名字 
		  // 映射到 实际的 类别id
...
///////////////////////////////////////////////////////
// 解析 coco  的 json格式的 标注文件 到 anno_datum=============
// Parse MSCOCO detection annotation.
bool ReadJSONToAnnotatedDatum(
const string& labelfile, 
...
bool ReadTxtToAnnotatedDatum(
const string& labelfile, // txt标注文件
const int height,// 原图像尺寸
const int width, 
AnnotatedDatum* anno_datum) 
{
