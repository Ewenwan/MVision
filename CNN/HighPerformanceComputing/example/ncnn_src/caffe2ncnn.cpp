...
// 用caffe自带的工具，将老版本转换为新版本
// ./upgrade_net_proto_text old_deploy.prototxt new_deploy.prototxt
// ./upgrade_net_proto_binary old.caffemodel new.caffemodel
// 开发版本的 prototxt 的数据层为　1张图片，确保　data layer　的 input_param 
// 的第一个　dim维度为１，表示图片数量

// 需要为 caffe 最新版本
    const char* caffeproto = argv[1];    // caffe 训练好的模型框架文件　xxx.prototxt
    const char* caffemodel = argv[2];  //  caffe 训练好的模型权重文件　xxx.caffemodel
    
    const char* ncnn_prototxt = argc >= 5 ? argv[3] : "ncnn.proto";// 转换后的　ncnn模型参数文件
    const char* ncnn_modelbin = argc >= 5 ? argv[4] : "ncnn.bin"; // 转换后的　ncnn 模型权重文件
    
    const char* quantize_param = argc >= 6 ? argv[5] : "0";             // 量化等级
    const char* int8scale_table_path = argc == 7 ? argv[6] : NULL; // 每一层　8位 int量化，尺度
    int quantize_level = atoi(quantize_param);// 量化等级, atoi()字符串转换成整数; atof()字符串转换成浮点数　

    if (quantize_level != 0 && quantize_level != 256 && quantize_level != 65536) 
    {// 量化成整数0/1  8位 2^8=256  16位 2^16=65536
        fprintf(stderr, "%s: only support quantize level = 0, 256, or 65536", argv[0]);
        return -1;
    }

    caffe::NetParameter proto;// 网络参数1
    caffe::NetParameter net;    // 网络参数2

// load载入 caffe 训练好的模型框架文件　xxx.prototxt
...
//  load载入 caffe 训练好的模型权重文件　xxx.caffemodel
...
// 载入 量化尺度参数
    std::map<std::string, std::vector<float> > blob_int8scale_table;    // FeatureMap的量化尺度
    std::map<std::string, std::vector<float> > weight_int8scale_table;// 卷积参数的量化尺度
...
// 打开需要写入的文件　
    FILE* pp = fopen(ncnn_prototxt, "wb");    // 转换后的　ncnn模型参数文件
    FILE* bp = fopen(ncnn_modelbin, "wb"); // 转换后的　ncnn 模型权重文件

    // magic  先写入一个魔数
    fprintf(pp, "7767517\n");

    // rename mapping for identical bottom top style
    std::map<std::string, std::string> blob_name_decorated;//  每一层的  每一个输入 通道blob 的 名字

    // bottom blob reference
    std::map<std::string, int> bottom_reference;//  输入 通道blob 的 名字 的 使用数量 
                                                                               // 某些BLOB会进入多个层, 使用了多次, 需要split

    // global definition line
    // [layer count] [blob count]
    int layer_count = proto.layer_size();//  模型网络 层数量
    std::set<std::string> blob_names;   //  输入 通道blob 的 名字  序列　链表　　
    for (int i=0; i<layer_count; i++)// 每一层
    {
        const caffe::LayerParameter& layer = proto.layer(i);// 网络参数的每一层为层参数

        for (int j=0; j<layer.bottom_size(); j++)// 每一层的  每一个输入 通道
        {
            std::string blob_name = layer.bottom(j);// 每一个输入 通道　blob 的 名字
            if (blob_name_decorated.find(blob_name) != blob_name_decorated.end())
            {
                blob_name = blob_name_decorated[blob_name];// 找到了
            }

            blob_names.insert(blob_name);// 输入 通道blob 的 名字  序列　链表　　

            if (bottom_reference.find(blob_name) == bottom_reference.end())
            {
                bottom_reference[blob_name] = 1; // 第一次记录
            }
            else
            {
                bottom_reference[blob_name] = bottom_reference[blob_name] + 1; // 使用数量+1 
            }
        }
 ...
 // 一个输入,一个输出，并且名字不变
        ...
        {
            std::string blob_name = layer.top(0) + "_" + layer.name();//通道名字+层名字　单一
            blob_name_decorated[layer.top(0)] = blob_name;
            blob_names.insert(blob_name);
        }
        else
        {
            for (int j=0; j<layer.top_size(); j++)// 每一层的每一个输出通道
            {
...
//             fprintf(stderr, "%s %d\n", it->first.c_str(), it->second);
    ...
    for (int i=0; i<layer_count; i++)
    {
        ...
        if (layer.type() == "BN")// 归一化层，
        {
            fprintf(pp, "%-16s", "Scale");// 标识, 　归一化 ---> Scale　 
        }
        else if (layer.type() == "Convolution")// 卷积层
        {
            const caffe::ConvolutionParameter& convolution_param = layer.convolution_param();// 该层的卷积参数
            if (convolution_param.group() != 1)
                fprintf(pp, "%-16s", "ConvolutionDepthWise");// 卷积分组　　分组卷积　逐通道卷积
            else
        ...
        else if (layer.type() == "Deconvolution") // 反卷积层
        {
            const caffe::ConvolutionParameter& convolution_param = layer.convolution_param();
            if (convolution_param.group() != 1)
                fprintf(pp, "%-16s", "DeconvolutionDepthWise");// 逐通道反卷积层
            else
                fprintf(pp, "%-16s", "Deconvolution");
        }
        else if (layer.type() == "MemoryData")// 内存数据
        {
	  ...
	  // faster-rcnn　中的　区域
            ...
            if (python_layer_name == "ProposalLayer")// 区域建议层
                fprintf(pp, "%-16s", "Proposal");// 区域建议层
            else
                fprintf(pp, "%-16s", python_layer_name.c_str());// 直接写入名字
        }
        else if (layer.type() == "ReLU6")// (0,x,6) 激活截断层
        {
            fprintf(pp, "%-16s", "Clip");// 激活截断层
        }
        else
        {
        ...
        for (int j=0; j<layer.bottom_size(); j++)
        {
            ...
            if (bottom_reference.find(blob_name) != bottom_reference.end())
            {
                ...
                sprintf(splitsuffix, "_splitncnn_%d", refidx);// 该 blob　分割的通路数
        ...
        if (layer.type() == "BatchNorm")
        {
            ...
            const caffe::BatchNormParameter& batch_norm_param = layer.batch_norm_param();// 批归一化层参数
            float eps = batch_norm_param.eps();// 方差归一化，分母，避免除零
        ...
        else if (layer.type() == "Concat") // 通道扩展层
        {
        ...
        else if (layer.type() == "Convolution" || layer.type() == "ConvolutionDepthwise" || layer.type() == "DepthwiseConvolution")
        {
            ...
            const caffe::BlobProto& weight_blob = binlayer.blobs(0); // 卷积w
            const caffe::ConvolutionParameter& convolution_param = layer.convolution_param();
            fprintf(pp, " 0=%d", convolution_param.num_output());// 输出通道数量
            if (convolution_param.has_kernel_w() && convolution_param.has_kernel_h())
            {
                fprintf(pp, " 1=%d", convolution_param.kernel_w());// 卷积和尺寸
            ...
            fprintf(pp, " 2=%d", convolution_param.dilation_size() != 0 ? convolution_param.dilation(0) : 1);// 空洞卷积参数======
            if (convolution_param.has_stride_w() && convolution_param.has_stride_h())// 卷积　步长=========
            {
            ...
            if (convolution_param.has_pad_w() && convolution_param.has_pad_h())// 卷积填充　===============
            {
            ...
            fprintf(pp, " 5=%d", convolution_param.bias_term()); // 卷积偏置 bias====
            fprintf(pp, " 6=%d", weight_blob.data_size());// 卷积核参数数量==========

            int num_group = 1;
            if (layer.type() == "ConvolutionDepthwise" || layer.type() == "DepthwiseConvolution")
            {
                num_group = convolution_param.num_output();// 逐通道卷积，即卷积分组数量为 通道数量
            }
            else
            {
            ...
            if (num_group != 1)
            {
                fprintf(pp, " 7=%d", num_group);// 记录分组数量
            }
            
//  载入量化参数=================================
            bool int8_scale_term = false;
            std::vector<float> weight_int8scale;// 卷积核参数 int8 量化尺度 
            std::vector<float> blob_int8scale;   // featureMap 量化尺度
            ...
            if (int8scale_table_path)
            {
                ...
                {
                    weight_int8scale = weight_int8scale_table[std::string(key)];// 卷积核参数 int8 量化尺度 
                }

                if (blob_int8scale_table.find(layer.name()) != blob_int8scale_table.end())
                {
                    blob_int8scale = blob_int8scale_table[layer.name()];// featureMap 量化尺度
                }
                ...
                if (int8_scale_term)
                {
                    if ((int)weight_int8scale.size() == num_group && (int)blob_int8scale.size() == num_group)// 多卷积核分组量化
                    {
                        fprintf(pp, " 8=1");
                    }
                    else
            ...
            for (int j = 0; j < binlayer.blobs_size(); j++)
            {
                ...
                std::vector<signed char> int8_weights;// 量化后的 模型权重文件

                // we will not quantize the bias values
                if (j == 0) // 不量化　偏置 bias（每个卷积层就一个 bias 参数 ）
                {
                    if (int8_scale_term)
                    {
                        if (quantize_level == 0)
                        ...
                        else if (quantize_level == 256) // 8bits量化=============== 
                        {
                    ...
                    else if (quantize_level == 65536) // 16bits 量化=============
                    {
                    ...
                    if (quantize_tag)
                    {
                        int p0 = ftell(bp); // 用于得到文件位置指针当前位置相对于文件首的偏移字节数
                        if (int8_scale_term)
                        {
                            if (quantize_level == 0) // 不量化======================
                            {
   ...
   //  32字节对齐
            ...
            if (int8_scale_term)
            {
                ...
                fwrite(weight_int8scale.data(), sizeof(float), weight_int8scale.size(), bp);// 同时记录　量化尺度参数
        ...
        else if (layer.type() == "DetectionOutput") // 目标检测输出层==================
        {
        ...
        else if (layer.type() == "Dropout")//  随机失活层
        {
            const caffe::DropoutParameter& dropout_param = layer.dropout_param();
            if (dropout_param.has_scale_train() && !dropout_param.scale_train())
            {
                float scale = 1.f - dropout_param.dropout_ratio(); // 保留比例=========
                fprintf(pp, " 0=%f", scale);
            }
        }
        else if (layer.type() == "Eltwise") // 组元素操作 有三个：product（点乘）， sum（相加减） 和 max（取大值），其中sum是默认操作。
        {
        ...
        else if (layer.type() == "InnerProduct")//  全连接层=====================================
        {
            const caffe::LayerParameter& binlayer = net.layer(netidx);
        ...
        else if (layer.type() == "Input") // 输入层=============================================
        {
            const caffe::InputParameter& input_param = layer.input_param();
            const caffe::BlobShape& bs = input_param.shape(0);// 维度
            if (bs.dim_size() == 4)
            {
        ...
        else if (layer.type() == "Permute")// ssd 　用于置换索引轴顺序的===================================
        {
        ...
        else if (layer.type() == "Pooling")// 池化层====================================
        {
        ...
        else if (layer.type() == "Power")// POWER层通过 (shift + scale * x) ^ power计算每一个输入x的输出
        {
        ...
        else if (layer.type() == "PReLU")// 参数Relu  (ax,x)
        {
            const caffe::LayerParameter& binlayer = net.layer(netidx);
        ...
        else if (layer.type() == "PriorBox")// 先验框======================================
        {
...
//                 float scale;
        ...
        else if (layer.type() == "ReLU6")// (0,x,6)=================
        {
            float min = 0.f;
            float max = 6.f;
            fprintf(pp, " 0=%f", min);
        ...
        else if (layer.type() == "Reorg")// yolo-v2　一拆四层=================
        {
        ...
        else if (layer.type() == "ROIPooling")// faster-rcnn　roi池化============
        {
        ...
        else if (layer.type() == "Scale")// bn后的缩放========================
        {
            const caffe::LayerParameter& binlayer = net.layer(netidx);
        ...
        else if (layer.type() == "ShuffleChannel")// 通道打乱=================================
        {
        ...
        else if (layer.type() == "Slice")// 拆分层===============与concat通道合并相反
        {
        ...
        else if (layer.type() == "Softmax")// 归一化   
        {
        ...
        else if (layer.type() == "YoloDetectionOutput")// yolo检测输出层======================================
        {
            const caffe::YoloDetectionOutputParameter& yolo_detection_output_param = layer.yolo_detection_output_param();

            fprintf(pp, " 0=%d", yolo_detection_output_param.num_classes());// 类别
            fprintf(pp, " 1=%d", yolo_detection_output_param.num_box());     // 框
            fprintf(pp, " 2=%f", yolo_detection_output_param.confidence_threshold());// 置信度
            fprintf(pp, " 3=%f", yolo_detection_output_param.nms_threshold());// nms非极大值抑制　剔除重叠框 阈值
