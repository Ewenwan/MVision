...
{
    int size = w * h;//每个通道数据数量
// 1. 只减去均值======================================
    if (mean_vals && !norm_vals)
    {
        // substract mean only
        #pragma omp parallel for
        for (int q=0; q<c; q++)// 每个通道
        {
            float* ptr = channel(q);//data + cstep * q;
            const float mean = mean_vals[q];// 该通道均值

#if __ARM_NEON
            // 4字节内存对齐后还剩余多少
            int nn = size >> 2;// 除以4
            int remain = size - (nn << 2);// 四字节对其后剩余数量
#else
            int remain = size;
#endif // __ARM_NEON
...
// 2. 值归一化========================================
    else if (!mean_vals && norm_vals)
    {
        ...
        {
            float* ptr = channel(q);//data + cstep * q;
            const float norm = norm_vals[q];// 归一化 因数

#if __ARM_NEON
            int nn = size >> 2;// 4字节内存对其
...
// float16 的 data 转换成 float32 的 data ==================
...
{
    double scale_x = (double)src.w / w; // 缩放尺度
