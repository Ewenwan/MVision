...
# python的yield详解 https://www.ibm.com/developerworks/cn/opensource/os-cn-python-yield/
##读取打印第一个随机数据块
...
for param in params:
    param.attach_grad()#自动求微分/求导 得到梯度 param.grad
...
for e in range(epochs):
    total_loss = 0#总的loss
    for data, label in data_iter():
        with autograd.record():##自动微分 求解梯度
            output = net(data)#输出
