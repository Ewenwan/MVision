# coding=utf-8
# 处理coco数据集  得到darknet训练所需的 数据集格式 标签
# 使用说明
# 需要先安装coco tools
...
# 将ROI的坐标转换为yolo需要的坐标
# size是图片的w和h
# box里保存的是ROI的坐标（左上角坐标值(x,y)和边框的宽和高）
...
def get_classes_and_index(path):
    ...
    f = open(path)# 文件地址
    for line in f:# 每一行
        temp = line.rstrip().split(',', 2)#截取前两个
        print("temp[0]:" + temp[0] + "\n")
        print("temp[1]:" + temp[1] + "\n")
        D[temp[1]] = temp[0]#类名  类别名字 
    return D


dataDir = '/Data/COCO'  # COCO数据集所在的路径
dataType = 'train2014'  # 要转换的COCO数据集的子集名
annFile = '%s/annotations/instances_%s.json' % (dataDir, dataType)  # COCO数据集的标注文件路径
# coco data 类别文件名
coco_class_name_file = '/darknet/data/coco_list.txt'
classes = get_classes_and_index(coco_class_name_file)
...
if not os.path.exists('%s/%s/labels/' % (dataDir, dataType)):
    ...
    shutil.rmtree('%s/%s/labels/' % (dataDir, dataType))# 清空目录
    os.makedirs('%s/%s/labels/' % (dataDir, dataType))#创建labels目录
...
catIds = coco.getCatIds()  # 获取标注文件中 所有的物体类别的COCO Cat ID
