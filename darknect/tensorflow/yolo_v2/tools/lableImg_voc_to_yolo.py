...
def convert_annotation(path, image_id):
    ...
    for obj in root.iter('object'):
        ...
        b = (float(xmlbox.find('xmin').text), float(xmlbox.find('xmax').text), float(xmlbox.find('ymin').text),
             float(xmlbox.find('ymax').text))#（左上角坐标值(x,y)和右下角坐标值）
        bb = convert((w, h), b)
...
wd = getcwd()
# voc 数据集 类别路径
voc_class_file = '/raid/pengchong_data/Tools/Paul_YOLO/data/Paul_list.txt'
...
for time in sets:
    ...
    for path in dirs:
        print(path)
        if not os.path.exists('%s/annotations/' % path):# 不存在
            os.makedirs('%s/annotations/' % path)# 创建
        if not os.path.exists('%s/labels/' % path):
            os.makedirs('%s/labels/' % path)
        else:
            shutil.rmtree('%s/labels/' % path)# 存在 清空
            os.makedirs('%s/labels/' % path)# 再创建
