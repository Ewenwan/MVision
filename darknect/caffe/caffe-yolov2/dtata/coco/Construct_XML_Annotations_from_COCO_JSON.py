...
"""
# python Construct_XML_Annotations_from_COCO_JSON.py -y 2017 -t train -c instances 
# python Construct_XML_Annotations_from_COCO_JSON.py -y 2017 -t val -c instances 
import json
...
parser.add_argument('-y', '--year',
                    ...
                    type=int,
                    required=True)
# 类型 训练或者验证 train' or 'val' 
parser.add_argument('-t', '--type',
                    help="The type of imageset, 'train' or 'val'",
                    choices=["train", "val"],
                    type=str, required=True)
# 标注文件类型 instances/captions/person_keypoints
...
# For example this default expects: $HOME/data/coco/Images/train2017   val2017
...
# 主数据集目录
ROOT_COCO = "./coco2017/coco/"

# 输出xml格式标注文件 Will create annotations/val or annotations/train
# OUTPUT_XML_BASE_DIR = "%s/data/coco/Annotations/" % (environ['HOME'])
OUTPUT_XML_BASE_DIR = "%sannotations/" % (ROOT_COCO)

# 输入JASON格式标注文件路径 sometimes we just have everything in one directory as silly as that sounds
#INPUT_JSON_BASE_DIR = "%s/data/coco/Annotations/" % (environ['HOME'])
INPUT_JSON_BASE_DIR = "%sannotations/" % (ROOT_COCO)

# 图片路径
PATH_TO_IMAGES = "%sImages/%s%d/" % ( ROOT_COCO, TYPEPREFIX, YEAR)

#PATH_TO_IMAGES = "%s/data/coco/Images/%s%d/" % ( environ['HOME'], TYPEPREFIX, YEAR)
...
if TYPEPREFIX == 'train':
    ...
    with open('val2.txt', 'w') as val_file:
        ...
        if not path.isdir(VALL_FULL_ANN_DIR):
            mkdir(VALL_FULL_ANN_DIR)
        for i in range(len(IMAGE_NAMES)):
            val_file.write(
