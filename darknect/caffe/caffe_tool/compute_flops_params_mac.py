...
def print_net_parameters_flops (deploy_file):
    ...
    for layer_name, blob in net.blobs.items():
        ...
        if net.layer_dict[layer_name].type in typenames:
            ...
            # 3*3卷积
            #if (net.params[layer_name][0].data.shape[2] == 3) and \
            #   (net.params[layer_name][0].data.shape[2] == net.params[layer_name][0].data.shape[3]):
            #    
            # 1*1卷积
            #if (net.params[layer_name][0].data.shape[2] == 1) and \
            #   (net.params[layer_name][0].data.shape[2] == net.params[layer_name][0].data.shape):
            #  
            h_in = h_out
            w_in = w_out
            # 打印当前层信息
