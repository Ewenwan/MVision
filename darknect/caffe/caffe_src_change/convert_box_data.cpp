// 添加 caffe\tools\convert_box_data.cpp
...
#include "caffe/util/io.hpp"//这里需要改动 添加一个函数
...
// 读入标签文件
  ...
  std::map<std::string, int> label_map;// key: value
  std::string tmp_line;
  ...
  std::ifstream infile(argv[2]);// 图片文件地址
  std::vector<std::pair<std::string, std::string> > lines;
  //图片地址 + yaml标签地址
  std::string line;
  size_t pos;
  while (std::getline(infile, line)) {// 每一行
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (encoded && !enc.size()) {
     ...
     // #include "caffe/util/io.hpp"//这里需要改动 添加一个函数
    // ReadImageToDatum ---> ReadBoxDataToDatum
